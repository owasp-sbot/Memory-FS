...
class test_Schema__Memory_FS__File__Content(TestCase):

    def test___init__(self):
        with Schema__Memory_FS__File__Content() as _:
            assert type(_) == Schema__Memory_FS__File__Content
            assert _.obj() == __(size=None, encoding=None, content_path=None)

        kwargs = dict(size         = 123        ,
                      encoding     = 'UTF_8'    ,
                      content_path = 'an/path'  )
        with Schema__Memory_FS__File__Content(**kwargs) as _:
            assert _.obj() == __(size         = 123,
                                 encoding     = 'UTF_8',
                                 content_path = 'an/path')
            assert type(_.size        ) is Safe_UInt__FileSize
            assert type(_.encoding    ) is Enum__Memory_FS__File__Encoding
